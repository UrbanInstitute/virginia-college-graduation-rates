...
const tabItems = comparisons.map((comparison, i) =>
  <a key={i} className="nav-link vertical-text" id={`tabs-${comparison.value}`} data-toggle="pill" href={`#tab-${comparison.value}`} role="tab" aria-controls={`tabs-${comparison.value}`}>{comparison.label}</a>
)

const tabOptions = comparisons.map((comparison, i) =>
  <option key={i} value={comparison.value}>{comparison.label}</option>
)
